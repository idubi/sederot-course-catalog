...
- **Implementation:** In progress — added an immutable-hash-checked baseline migrator and direct editor migration for legacy `groups` JSON. The editor accepts DOCX directly through its loopback API, extracts headings, paragraphs, lists, and tables in memory without modifying the source, and converts DOCX or Markdown/text Blueprint documents into the closest canonical editable draft with structured diagnostics. It also explicitly exports schema-valid development profiles to `contents/<folder>/bootstrap.json`. `npm run dev -- --data=./contents/<folder>` loads only that exact file for local development; traversal, symlink escape, and production use are rejected. The deterministic baseline draft contains all 2 programs, 26 audience groups, 21 courses, and 123 offerings; structured diagnostics expose all unresolved schema/content blockers. Description sanitation remains behind the loopback API so Node-only dependencies never enter the browser bundle. Approved content remains unchanged.
